...
//! and font metrics. Drawing is delegated to grid_renderer via Direct2D.
...
pub struct TerminalView {
    ...
    // HWND handle (set by view.rs when attached)
...
impl TerminalView {
    pub fn new(rows: usize, cols: usize) -> Self {
        ...
        let renderer = GridRenderer::new("Consolas", 14.0);
    ...
    /// Draw the entire terminal grid into a Direct2D render target.
    /// Takes `ID2D1RenderTarget` (the base interface) so all rendering methods
    /// are directly available. The caller casts from `ID2D1HwndRenderTarget`.
    pub fn draw(&self, rt: &ID2D1RenderTarget) {
        ...
        unsafe {
            ...
            let bg_brush = rt.CreateSolidColorBrush(
                &color_f(self.bg_color.0, self.bg_color.1, self.bg_color.2, 1.0),
                None,
            );
            let bg_brush = match bg_brush {
                Ok(b) => b,
                Err(_) => return,
            };
            rt.FillRectangle(
                &d2d_rect(0.0, 0.0, self.cols as f32 * cell_w as f32, self.rows as f32 * cell_h as f32),
                &bg_brush,
            );
            ...
            for row in 0..self.rows {
                ...
                let y = row as f32 * cell_h as f32;

                for col in 0..self.cols {
                    if col >= self.cells[row].len() { break; }
                    let cell = &self.cells[row][col];
                    let x = col as f32 * cell_w as f32;
                    ...
                    {
                        let cell_brush = rt.CreateSolidColorBrush(
                            &color_f(bg.0, bg.1, bg.2, 1.0),
                            None,
                        );
                        if let Ok(brush) = cell_brush {
                            let w = if cell.w == 2 { cell_w as f32 * 2.0 } else { cell_w as f32 };
                            rt.FillRectangle(&d2d_rect(x, y, x + w, y + cell_h as f32), &brush);
                        }
                    }
                }
            }
            ...
            for sel in &self.selections {
                ...
                if let Ok(brush) = sel_brush {
                    for row in sel.start_row..=sel.end_row {
                        ...
                        let x = col_start as f32 * cell_w as f32;
                        let w = (col_end - col_start) as f32 * cell_w as f32;
                        rt.FillRectangle(&d2d_rect(x, y, x + w, y + cell_h as f32), &brush);
                    }
                }
            }
            ...
            for row in 0..self.rows {
                ...
                for col in 0..self.cols {
                    ...
                    self.renderer.draw_char(rt, &cell.c, x, y, fg, cell.b, cell.i);

                    // Draw underline
                    if cell.u {
                        let underline_y = y + font.ascent + 2.0;
                        let line_brush = rt.CreateSolidColorBrush(
                            &color_f(fg.0, fg.1, fg.2, 1.0),
                            None,
                        );
                        if let Ok(brush) = line_brush {
                            match cell.us.as_deref() {
                                Some("double") => {
                                    rt.DrawLine(
                                        d2d_point(x as f32, underline_y as f32),
                                        d2d_point((x + cell_w) as f32, underline_y as f32),
                                        &brush, 1.0, None,
                                    );
                                    rt.DrawLine(
                                        d2d_point(x as f32, (underline_y + 2.0) as f32),
                                        d2d_point((x + cell_w) as f32, (underline_y + 2.0) as f32),
                                        &brush, 1.0, None,
                                    );
                                }
                                Some("curly") => {
                                    ...
                                    let mut prev = d2d_point(x as f32, underline_y as f32);
                                    for s in 0..segments {
                                        let sx = x + (s as f64 + 0.5) * seg_w;
                                        let sy = underline_y + if s % 2 == 0 { -1.5 } else { 1.5 };
                                        let next = d2d_point(sx as f32, sy as f32);
                                        rt.DrawLine(prev, next, &brush, 1.0, None);
                                        prev = next;
                                    }
                                    let end = d2d_point((x + cell_w) as f32, underline_y as f32);
                                    rt.DrawLine(prev, end, &brush, 1.0, None);
                                }
                                _ => {
                                    rt.DrawLine(
                                        d2d_point(x as f32, underline_y as f32),
                    ...
                    if cell.s {
                        ...
                        if let Ok(brush) = line_brush {
                            rt.DrawLine(
                                d2d_point(x as f32, strike_y as f32),
                                d2d_point((x + cell_w) as f32, strike_y as f32),
                                &brush, 1.0, None,
                            );
                        }
                    }
            ...
            if self.cursor.visible && self.cursor.row < self.rows && self.cursor.col < self.cols {
                let cx = self.cursor.col as f32 * cell_w as f32;
                let cy = self.cursor.row as f32 * cell_h as f32;

                match self.cursor.style {
                    0 => {
                        // Block cursor
                        let cursor_brush = rt.CreateSolidColorBrush(
                            &color_f(self.cursor_color.0, self.cursor_color.1, self.cursor_color.2, 0.7),
                            None,
                        );
                        if let Ok(brush) = cursor_brush {
                            rt.FillRectangle(
                                &d2d_rect(cx, cy, cx + cell_w as f32, cy + cell_h as f32),
                                &brush,
                            );
                        }
                    ...
                    1 => {
                        ...
                        let cursor_brush = rt.CreateSolidColorBrush(
                            &color_f(self.cursor_color.0, self.cursor_color.1, self.cursor_color.2, 1.0),
                            None,
                        );
                        if let Ok(brush) = cursor_brush {
                            rt.FillRectangle(
                                &d2d_rect(cx, cy, cx + 2.0, cy + cell_h as f32),
                                &brush,
                            );
                        }
                    }
                    2 => {
                        ...
                        if let Ok(brush) = cursor_brush {
                            rt.FillRectangle(
                                &d2d_rect(cx, cy + cell_h as f32 - 2.0, cx + cell_w as f32, cy + cell_h as f32),
                                &brush,
                            );
                        }
                    }
...
/// Create a D2D1_COLOR_F from f64 components.
pub fn color_f(r: f64, g: f64, b: f64, a: f64) -> D2D1_COLOR_F {
    D2D1_COLOR_F {
        r: r as f32,
        g: g as f32,
        b: b as f32,
...
/// Create a D2D_POINT_2F.
pub fn d2d_point(x: f32, y: f32) -> windows::Win32::Graphics::Direct2D::Common::D2D_POINT_2F {
    windows::Win32::Graphics::Direct2D::Common::D2D_POINT_2F { x, y }
}
