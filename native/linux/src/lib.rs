//! Linux FFI entry point for hone-terminal.
//!
...
/// Creates its own X11 window, populates the grid, and displays it.
/// Blocks — runs the X11 event loop until the window is closed.
#[no_mangle]
...
fn hone_terminal_show_demo_impl() {
    ...
    populate_demo_cells(&mut terminal_view, cols);

    // Run the demo window (blocking event loop)
    view::run_demo_window(terminal_view);
}
