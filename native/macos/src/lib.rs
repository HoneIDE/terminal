...
/// Creates its own NSWindow, populates the grid, and displays it.
/// Does NOT block — the window stays open and renders via the app's run loop.
#[no_mangle]
...
fn hone_terminal_show_demo_impl() {
    ...
    // The NSView's draw callback accesses TerminalView through a raw pointer.
    ...
    unsafe {
        ...
        // Get the main screen's visible frame to position within it
